...
data CryptoActions (pr :: Bool) (ra :: Bool) (l :: [Type]) (bef :: Bool) (aft :: Bool) (a :: Type) where
  ...
  -- YieldAction :: InList (Maybe x, y) l -> a -> CryptoActions pr ra l True False a
...
-- yield :: InList (Maybe x, y) l -> CryptoMonad pr ra l True False ()
...
sendSync :: x -> CryptoMonad pr ra '[(x, y)] True True y
sendSync m = M.do
  send Here m
  recvAny >>=: \case
